	...
	<div class="box">
		<div class="head">
			...
			<div id="lanmu-content">
				...
				if($hqsql1['guanli']==1){
					if($saixuan){
						$sql="select * from pengyou_content where shenhe=0 ORDER BY time desc limit $page,20";	
					}else{
						$sql="select * from pengyou_content ORDER BY time desc limit $page,20";	
					}
				...
				echo "<div class=\"lanmu-content\" id=\"daohang-buttom-0\">";
						...
						if($hqsql1['guanli']==1){
							while($hqsql=mysqli_fetch_assoc($zxsql)){
								...
								if($shenhe==0){
									echo "<input type=\"submit\" value=\"未审核\" onClick=\"shenhe($lmid)\" class='bg-hui' id=\"shenhe$lmid\">";
								}else{
									echo "<input type=\"submit\" value=\"已审核\" onClick=\"shenhe($lmid)\" class='bg-blu' id=\"shenhe$lmid\">";
								}
								if($hqsql2['guanli']==3){
									echo "<input type=\"submit\" value=\"已拉黑\" onClick=\"lahei($userid)\" class='bg-hui'>";
								}else{
									echo "<input type=\"submit\" value=\"拉黑\" onClick=\"lahei($userid)\" class='bg-black'>";
								}
								if($hqsql['zhiding']==1){
									echo "<input type=\"submit\" value=\"取消置顶\" onClick=\"zhidingqx($lmid)\" class='bg-sred' id=\"zhiding$lmid\">";
								}else{
									echo "<input type=\"submit\" value=\"置顶\" onClick=\"zhiding($lmid)\" class='bg-blu' id=\"zhiding$lmid\">";
								}
								
								echo "<input type=\"submit\" value=\"删除\" onClick=\"zlanmushanchu($lmid)\" class='bg-red'>";
							...
							$page_sql="select * from pengyou_content where shenhe=0";			
