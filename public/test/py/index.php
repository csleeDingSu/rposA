...
<body id='Body'>
			...
			<div class="pengyou-headimg" style="background-image:url(<?php echo $bg ?>);background-repeat:no-repeat;background-size:100% 86%;">
				<div class="pengyou-touxiang">
					...
					<?php 
						...
						if($pengyou_tximg){
							echo '<img src="images/touxiang/'.$pengyou_tximg.'" onclick="uploadtouxiang()">';
						}else{
	...
	<?php 			
				...
				if($shenhe==1){
					$sql='select * from pengyou_content where shenhe=1 order by if(zhiding=1 and zhidingtime>"'.date('Y-m-d H:i').'",0,1),time desc limit 10';
				}else{
					//$sql='select * from pengyou_content ORDER BY time desc limit 10';
					$sql='select * from pengyou_content order by if(zhiding=1 and zhidingtime>"'.date('Y-m-d H:i').'",0,1),time desc limit 10';
				}
					...
					while($hqsql=mysqli_fetch_assoc($zxsql)){
						...
						if($hqsql['zhiding']==1 and $hqsql['zhidingtime']>date('Y-m-d H:i')){
							echo '<div class="pengyou-zhiding">置顶</div>';
						}
						echo '</div>';
						...
						if($name){
							if($qqvip==6 || $qqvip==5){
								echo '<p title="" onclick="Dqopenuser('.$hqplzid.')" style="color:#f44336;">'.$name.'</p>';
							}else{
