var being = {
	...
	respond: function(width) {
		...
		var autoW = _w * 100 / width;
		$('html').css('font-size', autoW);
	},
	//字体大小
	...
	loadShow: function(time, title, callback) {
		var that = this;
		var html = '<div class="loadIng"><div class="inBox dflex"><img src="../images/Loading.gif">';
		html += '<p>' + title + '</p>';
		html += '</div></div>';
	...
	success: function(title, callback) {
		...
		$('.successBox').find('.sureBtn').click(function() {
			that.wrapHide();
			that.successHide(callback);
		});

		$('.successBox').find('.closeBtn').click(function() {
			that.wrapHide();
			that.successHide();
		});
	},
	...
	wrong: function(title, callback) {
		var that = this;
		var html = '<div class="wrongBox scaleHide dflex-2"><div class="inBox"><p>' + title + '</p></div></div>';
		$('.card').append(html);
		that.wrapShow('.card');
	...
	scrollBottom: function(a, b, callback) {
		...
		if(a) {
			a.scroll(function() {
				...
				console.log(wHeight + ':::' + dHeight + ":::" + sHeight + '::' + a.scrollTop());
				if(sHeight > 0) {
	...
	tabBar: function(hname, bname) {
		...
		hn.click(function() {
			...
			bn.children().eq(i).fadeIn('150').siblings().hide(0);
		});
	},
	run: function() {
		var that = this;
