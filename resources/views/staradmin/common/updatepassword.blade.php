...
<div class="container-scroller">
              ...
              <div class="auto-form-wrapper">
				  <div class="alert alert-info alert-dismissable" id="updatemsg">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">
					 ...
					 @lang('dingsu.reset_password_instruction')
				  </div>
                <form action="#" method="post" name="resetpasswordform" id="resetpasswordform" autocomplete="off">
                  <div class="form-group">
                    <input type="hidden" name="resettoken" id="resettoken" value="{{ request()->segment('3') }}">
                    <div class="input-group">
                      <input name="password" id="password" type="text" class="form-control" placeholder="@lang('dingsu.password')" autofocus required>
                  ...
                  <div class="form-group">
                    ...
                    <div class="input-group">
                      <input name="password_confirmation" id="password_confirmation" type="text" class="form-control" placeholder="@lang('dingsu.confirm_password')"  required>
                      <div class="input-group-append">
                        <span class="input-group-text">
                          <i class="mdi mdi-check-circle-outline"></i>
                        </span>
                  ...
                  <div class="form-group">
                    <button id="resetbutton" type="submit" class="btn btn-primary submit-btn btn-block">@lang('dingsu.reset')&nbsp;@lang('dingsu.password')</button>
                  </div>
	...
	<script type="text/javascript">
	 $('#resetpasswordform').on('submit', function(e){
		...
		jQuery.ajax({
			type:"POST",
			url: "{{route('member.update.password')}}",
			data:{
				_token: "{{ csrf_token() }}",
			...
			success:function(data){
				if (data.success == true) 
					{
						$( "div#updatemsg" ).removeClass('alert-info alert-danger').addClass('alert-success').html( '@lang("dingsu.reset_password_success_message")' );
					}
				else 
					{
			...
			error: function (xhr, ajaxOptions, thrownError) {
				$("#resetbutton").text('@lang("dingsu.reset")');
     			$('#resetbutton').removeAttr('disabled');
				$( "div#updatemsg" ).removeClass('alert-info').addClass('alert-danger').html(ajaxOptions +' : '+ thrownError );
				//alert(thrownError);
			}
		});
	});
