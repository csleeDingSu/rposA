...
// ---------------------Credit--------------------------------------
	...
	*/
	...
	public function usedpoint_103(Request $request)
    {
		...
		$point   = \DB::table('a_view_used_point_list')->selectRaw('*, sum(debit) as point')->where('game_id',103)->where('ledger_type','DPRBP')->groupby('member_id')->orderby('point')->paginate(30);
