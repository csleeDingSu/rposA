...
use App\Package;
class RedisGameController extends Controller
{
	...
	public function master_out(Request $request)
	{
		...
		if (empty($memberid)) die('unknown member');
		//echo 'here';
		//die();
		...
		//$setting     = \App\Admin::get_setting();
		//$data =  response()->json(['success' => true, 'gamesetting' => $gamesetting, 'gamenotification' => $gamenotific,'bethistory' => $bethistory,'gamehistory' => $gamehistory,'game_temp' => $game_temp,'wallet' => $wallet,'futureresults' => $futureresult,'wabaofee' => $setting->wabao_fee]);
		
		//$data =  response()->json(['gamesetting' => $gamesetting,'wallet' => $wallet]);
		$data =  ['gamesetting' => $gamesetting,'wallet' => $wallet];
		
		//print_r($data);
		event(new \App\Events\EventBetting($memberid,$data));
		///event(new \App\Events\TestEvent(73));
		//event(new \App\Events\MasterGameResult($data, $memberid));
